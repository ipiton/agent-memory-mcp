...
func TestLoadFromEnvEmbeddingModeLocalOnly(t *testing.T) {
	...
	if cfg.Embeddings.Mode != "local-only" {
		t.Fatalf("EmbeddingMode = %q, want %q", cfg.Embeddings.Mode, "local-only")
	}
}
...
func TestLoadFromEnvEmbeddingTuningDefaults(t *testing.T) {
	...
	if cfg.Embeddings.Timeout != 5*time.Second {
		t.Fatalf("EmbeddingTimeout = %s, want 5s", cfg.Embeddings.Timeout)
	}
	if cfg.Embeddings.MaxRetries != 1 {
		t.Fatalf("EmbeddingMaxRetries = %d, want 1", cfg.Embeddings.MaxRetries)
	}
...
func TestLoadFromEnvEmbeddingTuningOverrides(t *testing.T) {
	...
	if cfg.Embeddings.Timeout != 30*time.Second {
		t.Fatalf("EmbeddingTimeout = %s, want 30s", cfg.Embeddings.Timeout)
	}
	if cfg.Embeddings.MaxRetries != 3 {
		t.Fatalf("EmbeddingMaxRetries = %d, want 3", cfg.Embeddings.MaxRetries)
	}
...
func TestLoadFromEnvLlamaCPPDisabledByDefault(t *testing.T) {
	...
	if cfg.Embeddings.LlamaCPPBaseURL != "" {
		t.Fatalf("LlamaCPPBaseURL = %q, want empty (opt-in)", cfg.Embeddings.LlamaCPPBaseURL)
	}
	// Model still has a default so the adapter has a value once enabled.
	if cfg.Embeddings.LlamaCPPModel != "bge-m3" {
		t.Fatalf("LlamaCPPModel = %q, want bge-m3", cfg.Embeddings.LlamaCPPModel)
	}
...
func TestLoadFromEnvLlamaCPPLocalOnlyOverrides(t *testing.T) {
	...
	if cfg.Embeddings.Mode != "local-only" {
		t.Fatalf("EmbeddingMode = %q, want local-only", cfg.Embeddings.Mode)
	}
	ec := cfg.EmbedderConfig()
...
func TestLoadFromEnvHTTPDefaults(t *testing.T) {
	...
	if cfg.HTTP.Host != "127.0.0.1" {
		t.Fatalf("HTTPHost = %q, want %q", cfg.HTTP.Host, "127.0.0.1")
	}
	if cfg.HTTP.Port != 18080 {
		t.Fatalf("HTTPPort = %d, want %d", cfg.HTTP.Port, 18080)
	}
	if cfg.HTTP.InsecureAllowUnauthenticated {
		t.Fatal("HTTPInsecureAllowUnauthenticated = true, want false")
	}
	if !cfg.Session.TrackingEnabled {
		t.Fatal("SessionTrackingEnabled = false, want true")
	}
	if cfg.Session.IdleTimeout != 10*time.Minute {
		t.Fatalf("SessionIdleTimeout = %s, want 10m", cfg.Session.IdleTimeout)
	}
	if cfg.Session.CheckpointInterval != 30*time.Minute {
		t.Fatalf("SessionCheckpointInterval = %s, want 30m", cfg.Session.CheckpointInterval)
	}
	if cfg.Session.MinEvents != 2 {
		t.Fatalf("SessionMinEvents = %d, want 2", cfg.Session.MinEvents)
	}
}
...
func TestLoadFromEnvHTTPOverrides(t *testing.T) {
	...
	if cfg.HTTP.Host != "0.0.0.0" {
		t.Fatalf("HTTPHost = %q, want %q", cfg.HTTP.Host, "0.0.0.0")
	}
	if cfg.HTTP.Port != 28080 {
		t.Fatalf("HTTPPort = %d, want %d", cfg.HTTP.Port, 28080)
	}
	if !cfg.HTTP.InsecureAllowUnauthenticated {
...
func TestLoadFromEnvSessionTrackingOverrides(t *testing.T) {
	...
	if cfg.Session.IdleTimeout != 2*time.Minute {
		t.Fatalf("SessionIdleTimeout = %s, want 2m", cfg.Session.IdleTimeout)
	}
	if cfg.Session.CheckpointInterval != 5*time.Minute {
		t.Fatalf("SessionCheckpointInterval = %s, want 5m", cfg.Session.CheckpointInterval)
	}
	if cfg.Session.MinEvents != 4 {
		t.Fatalf("SessionMinEvents = %d, want 4", cfg.Session.MinEvents)
	}
}
