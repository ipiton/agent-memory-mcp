...
func (a jinaAdapter) embed(text, task string) ([]float32, error) {
	...
	if err := a.embedder.postJSON(jinaEmbeddingsURL, map[string]string{"Authorization": "Bearer " + a.embedder.config.JinaToken}, payload, &response); err != nil {
		return nil, fmt.Errorf("jina AI request failed: %w", err)
	}
	return singleEmbeddingFromCompatibleResponse("jina AI", response)
}

func (a jinaAdapter) batchEmbed(texts []string, task string) ([][]float32, error) {
	payload := map[string]any{
		"input":           texts,
		"model":           "jina-embeddings-v3",
		"encoding_format": "float",
...
func (a openAIAdapter) embed(text, _ string) ([]float32, error) {
	payload := map[string]any{
		"input":      text,
		"model":      a.model(),
		"dimensions": a.embedder.Dimension,
	...
	var response compatibleEmbeddingsResponse
	if err := a.embedder.postJSON(a.endpoint(), map[string]string{"Authorization": "Bearer " + a.embedder.config.OpenAIToken}, payload, &response); err != nil {
		return nil, fmt.Errorf("OpenAI API request failed: %w", err)
	}
	return singleEmbeddingFromCompatibleResponse("OpenAI", response)
}

func (a openAIAdapter) batchEmbed(texts []string, _ string) ([][]float32, error) {
	payload := map[string]any{
		"input":      texts,
		"model":      a.model(),
		"dimensions": a.embedder.Dimension,
